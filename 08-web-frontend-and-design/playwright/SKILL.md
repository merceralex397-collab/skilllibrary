...
When the user explicitly asks for test files (not CLI automation), use `@playwright/test`:
...
- **Locators**: `page.getByRole()`, `page.getByLabel()`, `page.getByText()`, `page.getByTestId()` — prefer role-based locators
- **Assertions**: `expect(locator).toBeVisible()`, `.toHaveText()`, `.toHaveCount()`, `.toHaveURL()`
- **Fixtures**: `test.use({ storageState: 'auth.json' })` for authenticated tests
