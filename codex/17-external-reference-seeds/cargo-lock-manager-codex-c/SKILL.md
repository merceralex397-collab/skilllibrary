---
name: cargo-lock-manager-codex-c
description: A Rust-package maintenance seed focused on Cargo.lock handling and dependency discipline. Use this when the user or repo work clearly points at cargo lock manager or a task in the "External Reference Seed Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for blind copy-import of seed material without adapting it into a usable skill or workflow.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [cargo, lock, manager]
---

# Purpose

A Rust-package maintenance seed focused on Cargo.lock handling and dependency discipline.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at cargo lock manager
- a task in the "External Reference Seed Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around cargo lock manager

# Do not use this skill when

- the task is really about quick factual lookups with no synthesis, comparison, or decision support
- If the task is more specifically about `fastapi-patterns` or `bigquery-skill`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the exact question and decision boundary for Cargo Lock Manager.
