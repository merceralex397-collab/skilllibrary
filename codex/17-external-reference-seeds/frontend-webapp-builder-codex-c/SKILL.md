---
name: frontend-webapp-builder-codex-c
description: A broader external-style frontend builder seed that can feed more specific packs. Use this when the user or repo work clearly points at frontend webapp builder or a task in the "External Reference Seed Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for blind copy-import of seed material without adapting it into a usable skill or workflow.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [frontend, webapp, builder]
---

# Purpose

A broader external-style frontend builder seed that can feed more specific packs.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at frontend webapp builder
- a task in the "External Reference Seed Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around frontend webapp builder

# Do not use this skill when

- the task is really about quick factual lookups with no synthesis, comparison, or decision support
- If the task is more specifically about `gh-address-comments` or `linear-address-issue`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the exact question and decision boundary for Frontend Webapp Builder.
