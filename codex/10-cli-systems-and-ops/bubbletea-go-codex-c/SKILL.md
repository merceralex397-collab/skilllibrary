---
name: bubbletea-go-codex-c
description: Builds stateful terminal UIs in Go with Bubble Tea models, updates, and views. Use this when the user or repo work clearly points at bubble tea for go tuis or a task in the "CLI, Systems, and Ops Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for application logic changes that do not affect delivery, runtime, or operator workflow.
license: Apache-2.0
compatibility:
...
Builds stateful terminal UIs in Go with Bubble Tea models, updates, and views.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at bubble tea for go tuis
- a task in the "CLI, Systems, and Ops Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around bubble tea for go tuis

# Do not use this skill when

- the task is really about application logic changes that do not affect delivery, runtime, or operator workflow
- If the task is more specifically about `tui-development` or `cobra-go`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the target environment, promotion path, and operator constraints for Bubble Tea for Go TUIs.
