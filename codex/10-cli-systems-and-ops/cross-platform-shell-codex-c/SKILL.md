---
name: cross-platform-shell-codex-c
description: Covers shell and path differences across Linux, macOS, and Windows for tooling that should travel. Use this when the user or repo work clearly points at cross-platform shell or a task in the "CLI, Systems, and Ops Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for application logic changes that do not affect delivery, runtime, or operator workflow.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [cross, platform, shell]
---

# Purpose

Covers shell and path differences across Linux, macOS, and Windows for tooling that should travel.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at cross-platform shell
- a task in the "CLI, Systems, and Ops Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around cross-platform shell

# Do not use this skill when

- the task is really about application logic changes that do not affect delivery, runtime, or operator workflow
- If the task is more specifically about `config-files-xdg` or `packaging-installers`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the target environment, promotion path, and operator constraints for Cross-Platform Shell.
