---
name: tui-development-codex-c
description: Applies terminal UI patterns, event loops, layout, and keyboard-centric UX to text interfaces. Use this when the user or repo work clearly points at tui development or a task in the "CLI, Systems, and Ops Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for application logic changes that do not affect delivery, runtime, or operator workflow.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [tui, development]
---

# Purpose

Applies terminal UI patterns, event loops, layout, and keyboard-centric UX to text interfaces.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at tui development
- a task in the "CLI, Systems, and Ops Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around tui development

# Do not use this skill when

- the task is really about application logic changes that do not affect delivery, runtime, or operator workflow
- If the task is more specifically about `bash` or `terminal-debugging`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the target environment, promotion path, and operator constraints for TUI Development.
