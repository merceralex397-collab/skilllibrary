---
name: mcp-protocol-codex-c
description: Applies repo-specific MCP protocol, schema, and fail-closed policies. Use this when building, debugging, securing, or packaging an MCP server or MCP tool surface or a task in the "Generated Repo Core Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for adjacent tasks where a more specific skill already covers the job.
license: Apache-2.0
compatibility:
...
- a plan, ticket, or repo state would benefit from explicit guardrails around mcp protocol

# Do not use this skill when

- the task is really about adjacent tasks where a more specific skill already covers the job
- If the task is more specifically about `incident-postmortem` or `context-intelligence`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the task boundary for MCP Protocol.
