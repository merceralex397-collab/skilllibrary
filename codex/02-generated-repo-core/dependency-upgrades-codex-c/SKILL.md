---
name: dependency-upgrades-codex-c
description: Handles version bumps, lockfile churn, changelog review, and compatibility validation. Use this when the user or repo work clearly points at dependency upgrades or a task in the "Generated Repo Core Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for adjacent tasks where a more specific skill already covers the job.
license: Apache-2.0
compatibility:
  clients: [openai-codex, gemini-cli, opencode, github-copilot]
metadata:
  owner: codex
  domain: dependency-upgrades
  maturity: draft
  risk: low
  tags: [dependency, upgrades]
---
...
- the user or repo work clearly points at dependency upgrades
- a task in the "Generated Repo Core Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around dependency upgrades

# Do not use this skill when

- the task is really about adjacent tasks where a more specific skill already covers the job
- If the task is more specifically about `security-hardening` or `migration-refactor`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the task boundary for Dependency Upgrades.
