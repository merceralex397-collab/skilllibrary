---
name: context-intelligence-codex-c
description: Handles indexing, retrieval, provenance metadata, and cross-repo context intelligence surfaces. Use this when the user or repo work clearly points at context intelligence or a task in the "Generated Repo Core Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for adjacent tasks where a more specific skill already covers the job.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at context intelligence
- a task in the "Generated Repo Core Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around context intelligence

# Do not use this skill when

- the task is really about adjacent tasks where a more specific skill already covers the job
- If the task is more specifically about `release-engineering` or `incident-postmortem`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the task boundary for Context Intelligence.
