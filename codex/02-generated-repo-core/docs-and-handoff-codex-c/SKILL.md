---
name: docs-and-handoff-codex-c
description: Keeps README, process docs, ticket views, and restart documents aligned with canonical state. Use this when the user or repo work clearly points at docs and handoff or a task in the "Generated Repo Core Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for adjacent tasks where a more specific skill already covers the job.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at docs and handoff
- a task in the "Generated Repo Core Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around docs and handoff

# Do not use this skill when

- the task is really about adjacent tasks where a more specific skill already covers the job
- If the task is more specifically about `ticket-execution` or `review-audit-bridge`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the task boundary for Docs and Handoff.
