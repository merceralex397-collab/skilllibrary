---
name: drift-detection-codex-c
description: Checks whether implementation or repo state has drifted away from the approved plan or canonical brief. Use this when the user or repo work clearly points at drift detection or a task in the "Planning, Review, and Critique" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary implementation work where no review, critique, or comparison is needed.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at drift detection
- a task in the "Planning, Review, and Critique" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around drift detection

# Do not use this skill when

- the task is really about ordinary implementation work where no review, critique, or comparison is needed
- If the task is more specifically about `tradeoff-analysis` or `architecture-review`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the decision, plan, or implementation surface being reviewed for Drift Detection.
