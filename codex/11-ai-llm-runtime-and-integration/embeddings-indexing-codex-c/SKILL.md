---
name: embeddings-indexing-codex-c
description: Handles embedding model choice, indexing strategy, deduplication, and provenance-aware storage. Use this when the user or repo work clearly points at embeddings and indexing or a task in the "AI / LLM Runtime and Integration Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [embeddings, indexing]
---

# Purpose

Handles embedding model choice, indexing strategy, deduplication, and provenance-aware storage.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at embeddings and indexing
- a task in the "AI / LLM Runtime and Integration Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around embeddings and indexing

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `model-selection` or `rag-retrieval`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Embeddings and Indexing.
