---
name: llama-cpp-codex-c
description: Applies llama.cpp runtime patterns, quantization considerations, and local deployment habits. Use this when the user or repo work clearly points at llama.cpp or a task in the "AI / LLM Runtime and Integration Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
Applies llama.cpp runtime patterns, quantization considerations, and local deployment habits.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at llama.cpp
- a task in the "AI / LLM Runtime and Integration Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around llama.cpp

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `local-llm` or `ollama`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in llama.cpp.
