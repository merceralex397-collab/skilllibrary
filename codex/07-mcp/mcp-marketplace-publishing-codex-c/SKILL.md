...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [mcp, marketplace, publishing]
---

# Purpose

Packages MCP servers or skill bundles for publication, distribution, or catalog inclusion.
...
- a plan, ticket, or repo state would benefit from explicit guardrails around mcp marketplace publishing
...
- If the task is more specifically about `mcp-auth-transports` or `mcp-inspector-debugging`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the host, transport, and surface area involved in MCP Marketplace Publishing.
