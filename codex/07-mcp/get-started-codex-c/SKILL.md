---
name: get-started-codex-c
description: Provides a deterministic micro-skill for onboarding or initial validation of the local skill environment. Use this when the user or repo work clearly points at get started or a task in the "MCP Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for generic API integration work that does not involve MCP tools, resources, prompts, or host contracts.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at get started
- a task in the "MCP Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around get started
...
- If the task is more specifically about `mcp-builder` or `opencode-primitives`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the host, transport, and surface area involved in Get Started.
