...
Applies Go idioms and service structure to an MCP server implementation.
...
- a plan, ticket, or repo state would benefit from explicit guardrails around mcp go server
...
- If the task is more specifically about `mcp-python-fastmcp` or `mcp-typescript-sdk`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the host, transport, and surface area involved in MCP Go Server.
