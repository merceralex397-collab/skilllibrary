...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [mcp, inspector, debugging]
---

# Purpose

Uses inspector tooling, schema checks, logs, and host traces to debug MCP failures.
...
- a plan, ticket, or repo state would benefit from explicit guardrails around mcp inspector and debugging
...
- If the task is more specifically about `mcp-resources-prompts` or `mcp-auth-transports`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the host, transport, and surface area involved in MCP Inspector and Debugging.
