...
Applies TypeScript SDK patterns for MCP servers, tool registration, and runtime boundaries.
...
- a plan, ticket, or repo state would benefit from explicit guardrails around mcp typescript sdk
...
- If the task is more specifically about `mcp-host-integration` or `mcp-python-fastmcp`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the host, transport, and surface area involved in MCP TypeScript SDK.
