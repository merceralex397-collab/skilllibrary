---
name: opencode-primitives-codex-c
description: Holds exact OpenCode path, naming, and configuration primitives that other skills can assume. Use this when the user or repo work clearly points at opencode primitives or a task in the "MCP Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for generic API integration work that does not involve MCP tools, resources, prompts, or host contracts.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at opencode primitives
- a task in the "MCP Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around opencode primitives
...
- If the task is more specifically about `mcp-chatgpt-app-bridge` or `mcp-builder`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the host, transport, and surface area involved in OpenCode Primitives.
