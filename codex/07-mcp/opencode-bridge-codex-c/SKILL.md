---
name: opencode-bridge-codex-c
description: Explains how project-local skills and operating surfaces should bind into OpenCode-specific conventions. Use this when the user or repo work clearly points at opencode bridge or a task in the "MCP Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for generic API integration work that does not involve MCP tools, resources, prompts, or host contracts.
license: Apache-2.0
compatibility:
...
Explains how project-local skills and operating surfaces should bind into OpenCode-specific conventions.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at opencode bridge
- a task in the "MCP Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around opencode bridge
...
- If the task is more specifically about `opencode-primitives` or `get-started`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the host, transport, and surface area involved in OpenCode Bridge.
