---
name: analytics-instrumentation-codex-c
description: Defines events, funnels, naming, and capture rules so product analytics are useful rather than noisy. Use this when the user or repo work clearly points at analytics instrumentation or a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for backend-only or infrastructure-only work.
license: Apache-2.0
compatibility:
...
Defines events, funnels, naming, and capture rules so product analytics are useful rather than noisy.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at analytics instrumentation
- a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around analytics instrumentation

# Do not use this skill when

- the task is really about backend-only or infrastructure-only work
- If the task is more specifically about `webapp-testing` or `seo-structured-data`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Detect the actual UI stack, rendering constraints, and component boundaries involved in Analytics Instrumentation.
