---
name: react-native-firebase-codex-c
description: Captures a mobile app workflow pattern centered on React Native and Firebase integration. Use this when the user or repo work clearly points at react native firebase or a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for backend-only or infrastructure-only work.
license: Apache-2.0
compatibility:
...
Captures a mobile app workflow pattern centered on React Native and Firebase integration.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at react native firebase
- a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around react native firebase

# Do not use this skill when

- the task is really about backend-only or infrastructure-only work
- If the task is more specifically about `seo-structured-data` or `analytics-instrumentation`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Detect the actual UI stack, rendering constraints, and component boundaries involved in React Native Firebase.
