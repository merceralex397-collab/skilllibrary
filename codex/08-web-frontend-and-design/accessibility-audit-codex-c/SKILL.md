---
name: accessibility-audit-codex-c
description: Checks semantic structure, keyboard flow, contrast, labels, and assistive-tech friendliness. Use this when the user or repo work clearly points at accessibility audit or a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for backend-only or infrastructure-only work.
license: Apache-2.0
compatibility:
...
Checks semantic structure, keyboard flow, contrast, labels, and assistive-tech friendliness.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at accessibility audit
- a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around accessibility audit

# Do not use this skill when

- the task is really about backend-only or infrastructure-only work
- If the task is more specifically about `state-management` or `forms-validation`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Detect the actual UI stack, rendering constraints, and component boundaries involved in Accessibility Audit.
