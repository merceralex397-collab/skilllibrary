---
name: frontend-performance-codex-c
description: Looks at bundle size, loading strategy, rendering cost, image handling, and user-perceived responsiveness. Use this when the user or repo work clearly points at frontend performance or a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for backend-only or infrastructure-only work.
license: Apache-2.0
compatibility:
...
Looks at bundle size, loading strategy, rendering cost, image handling, and user-perceived responsiveness.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at frontend performance
- a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around frontend performance

# Do not use this skill when

- the task is really about backend-only or infrastructure-only work
- If the task is more specifically about `forms-validation` or `accessibility-audit`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Detect the actual UI stack, rendering constraints, and component boundaries involved in Frontend Performance.
