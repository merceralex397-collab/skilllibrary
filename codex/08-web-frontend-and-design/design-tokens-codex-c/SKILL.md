---
name: design-tokens-codex-c
description: Defines colors, spacing, typography, radius, and semantic token layers so style is consistent and reusable. Use this when the user or repo work clearly points at design tokens or a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for backend-only or infrastructure-only work.
license: Apache-2.0
compatibility:
...
Defines colors, spacing, typography, radius, and semantic token layers so style is consistent and reusable.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at design tokens
- a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around design tokens

# Do not use this skill when

- the task is really about backend-only or infrastructure-only work
- If the task is more specifically about `tailwind-shadcn` or `ux-design`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Detect the actual UI stack, rendering constraints, and component boundaries involved in Design Tokens.
