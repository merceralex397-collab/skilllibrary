---
name: forms-validation-codex-c
description: Handles form structure, validation, error display, and submission behavior in web apps. Use this when the user or repo work clearly points at forms and validation or a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for backend-only or infrastructure-only work.
license: Apache-2.0
compatibility:
...
Handles form structure, validation, error display, and submission behavior in web apps.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at forms and validation
- a task in the "Web, Frontend, and Design Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around forms and validation

# Do not use this skill when

- the task is really about backend-only or infrastructure-only work
- If the task is more specifically about `design-tokens` or `state-management`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Detect the actual UI stack, rendering constraints, and component boundaries involved in Forms and Validation.
