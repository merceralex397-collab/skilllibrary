...
Defines compute layout, storage, checkpointing, reproducibility, and orchestration for model training.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at training infrastructure
...
- a plan, ticket, or repo state would benefit from explicit guardrails around training infrastructure

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `model-merging` or `serving-architecture`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Training Infrastructure.
