---
name: eval-dataset-design-codex-c
description: Builds targeted evaluation sets that reflect the real task rather than generic benchmarks only. Use this when the user or repo work clearly points at eval dataset design or a task in the "AI / LLM Training, Architecture, and Research Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at eval dataset design
...
- a plan, ticket, or repo state would benefit from explicit guardrails around eval dataset design

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `quantization-research` or `inference-kernel-optimization`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Eval Dataset Design.
