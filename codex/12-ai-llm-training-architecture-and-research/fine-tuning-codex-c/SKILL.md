---
name: fine-tuning-codex-c
description: Covers when and how to fine-tune a model, what data is needed, and how to evaluate whether tuning helped. Use this when the work involves models, inference, training, evaluation, or LLM system design or a task in the "AI / LLM Training, Architecture, and Research Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
Covers when and how to fine-tune a model, what data is needed, and how to evaluate whether tuning helped.
...
- a plan, ticket, or repo state would benefit from explicit guardrails around fine tuning

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `instruction-tuning` or `preference-optimization`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Fine Tuning.
