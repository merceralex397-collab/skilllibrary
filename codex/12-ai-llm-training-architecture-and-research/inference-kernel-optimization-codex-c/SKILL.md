---
name: inference-kernel-optimization-codex-c
description: Looks at low-level math kernels, memory movement, and execution primitives for faster inference. Use this when the user or repo work clearly points at inference kernel optimization or a task in the "AI / LLM Training, Architecture, and Research Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [inference, kernel, optimization]
---

# Purpose

Looks at low-level math kernels, memory movement, and execution primitives for faster inference.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at inference kernel optimization
...
- a plan, ticket, or repo state would benefit from explicit guardrails around inference kernel optimization

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `distillation-compression` or `quantization-research`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Inference Kernel Optimization.
