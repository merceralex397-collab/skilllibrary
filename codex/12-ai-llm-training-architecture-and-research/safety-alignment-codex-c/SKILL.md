---
name: safety-alignment-codex-c
description: Covers risk shaping, refusal behavior, post-training controls, and alignment tradeoffs for model systems. Use this when the user or repo work clearly points at safety and alignment or a task in the "AI / LLM Training, Architecture, and Research Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at safety and alignment
...
- a plan, ticket, or repo state would benefit from explicit guardrails around safety and alignment

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `eval-dataset-design` or `benchmark-design`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Safety and Alignment.
