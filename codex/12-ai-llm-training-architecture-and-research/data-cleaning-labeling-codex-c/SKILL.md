---
name: data-cleaning-labeling-codex-c
description: Handles normalization, schema consistency, annotation quality, and label reliability for model data. Use this when the user or repo work clearly points at data cleaning and labeling or a task in the "AI / LLM Training, Architecture, and Research Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
Handles normalization, schema consistency, annotation quality, and label reliability for model data.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at data cleaning and labeling
...
- a plan, ticket, or repo state would benefit from explicit guardrails around data cleaning and labeling

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `synthetic-data-generation` or `dataset-curation`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Data Cleaning and Labeling.
