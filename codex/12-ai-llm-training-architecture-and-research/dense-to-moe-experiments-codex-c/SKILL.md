---
name: dense-to-moe-experiments-codex-c
description: Explores experimental methods for transforming dense-model behavior into expert-routed behavior without naive quality collapse. Use this when the user or repo work clearly points at dense-to-moe experiments or a task in the "AI / LLM Training, Architecture, and Research Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [dense, moe, experiments]
---

# Purpose

Explores experimental methods for transforming dense-model behavior into expert-routed behavior without naive quality collapse.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at dense-to-moe experiments
...
- a plan, ticket, or repo state would benefit from explicit guardrails around dense-to-moe experiments

# Do not use this skill when

- the task is really about ordinary software tasks with no model, inference, evaluation, or agent-runtime concerns
- If the task is more specifically about `serving-architecture` or `training-infrastructure`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the runtime goal, model boundaries, and interfaces involved in Dense-to-MoE Experiments.
