...
4. Choose sync versus async deliberately.
   Use `async def` only when the handler actually awaits async I/O. Keep CPU-heavy work or blocking libraries out of the event loop unless the repo already wraps them safely.
