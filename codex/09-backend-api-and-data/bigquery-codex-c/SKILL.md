---
name: bigquery-codex-c
description: Applies BigQuery-specific query, cost, and schema guidance when analytics or warehousing need it. Use this when the user or repo work clearly points at bigquery or a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for pure frontend styling or content-only tasks.
license: Apache-2.0
compatibility:
...
Applies BigQuery-specific query, cost, and schema guidance when analytics or warehousing need it.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at bigquery
- a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around bigquery

# Do not use this skill when

- the task is really about pure frontend styling or content-only tasks
- If the task is more specifically about `api-contracts` or `data-model`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Map the service boundary, data flow, and failure surface affected by BigQuery.
