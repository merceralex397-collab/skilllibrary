---
name: api-debugging-codex-c
description: A focused debugging pack for request/response mismatches, schema issues, and transport problems. Use this when the user or repo work clearly points at api debugging or a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for pure frontend styling or content-only tasks.
license: Apache-2.0
compatibility:
...
A focused debugging pack for request/response mismatches, schema issues, and transport problems.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at api debugging
- a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around api debugging

# Do not use this skill when

- the task is really about pure frontend styling or content-only tasks
- If the task is more specifically about `webhooks-events` or `realtime-websocket`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Map the service boundary, data flow, and failure surface affected by API Debugging.
