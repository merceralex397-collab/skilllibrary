---
name: background-jobs-queues-codex-c
description: Handles deferred work, retries, idempotency, schedule, and worker boundaries. Use this when the user or repo work clearly points at background jobs and queues or a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for pure frontend styling or content-only tasks.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at background jobs and queues
- a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around background jobs and queues

# Do not use this skill when

- the task is really about pure frontend styling or content-only tasks
- If the task is more specifically about `sqlite` or `orm-patterns`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Map the service boundary, data flow, and failure surface affected by Background Jobs and Queues.
