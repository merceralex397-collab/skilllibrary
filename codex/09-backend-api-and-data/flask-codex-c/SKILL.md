---
name: flask-codex-c
description: Applies Flask app structure, extension boundaries, and request handling patterns when Flask is chosen. Use this when the user or repo work clearly points at flask or a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for pure frontend styling or content-only tasks.
license: Apache-2.0
compatibility:
...
Applies Flask app structure, extension boundaries, and request handling patterns when Flask is chosen.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at flask
- a task in the "Backend, API, and Data Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around flask

# Do not use this skill when

- the task is really about pure frontend styling or content-only tasks
- If the task is more specifically about `python` or `fastapi`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Map the service boundary, data flow, and failure surface affected by Flask.
