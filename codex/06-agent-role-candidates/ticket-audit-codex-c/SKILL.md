---
name: ticket-audit-codex-c
description: Audits ticket state, dependencies, and artifact completeness for inconsistencies or stale transitions. Use this when the user or repo work clearly points at ticket audit or a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for tasks outside the narrow ticket audit role.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at ticket audit
- a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around ticket audit

# Do not use this skill when

- the task is really about tasks outside the narrow ticket audit role
- If the task is more specifically about `shell-inspection` or `context-summarization`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Restate the narrow role this skill is allowed to perform for Ticket Audit.
