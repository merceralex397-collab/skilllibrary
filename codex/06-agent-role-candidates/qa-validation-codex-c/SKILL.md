---
name: qa-validation-codex-c
description: Checks whether the work actually meets acceptance criteria from a tester/QA perspective. Use this when the user or repo work clearly points at qa validation or a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for tasks outside the narrow qa validation role.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at qa validation
- a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around qa validation

# Do not use this skill when

- the task is really about tasks outside the narrow qa validation role
- If the task is more specifically about `code-review` or `security-review`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Restate the narrow role this skill is allowed to perform for QA Validation.
