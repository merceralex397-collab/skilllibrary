---
name: implementer-hub-codex-c
description: Implements changes in the hub/core/service layer under an approved plan. Use this when the user or repo work clearly points at implementer hub or a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for tasks outside the narrow implementer hub role.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at implementer hub
- a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around implementer hub

# Do not use this skill when

- the task is really about tasks outside the narrow implementer hub role
- If the task is more specifically about `context-summarization` or `ticket-audit`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Restate the narrow role this skill is allowed to perform for Implementer Hub.
