---
name: shell-inspection-codex-c
description: Uses read-only terminal inspection to confirm repo state, file existence, and runtime facts. Use this when the user or repo work clearly points at shell inspection or a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for tasks outside the narrow shell inspection role.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at shell inspection
- a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around shell inspection

# Do not use this skill when

- the task is really about tasks outside the narrow shell inspection role
- If the task is more specifically about `repo-evidence-gathering` or `github-prior-art-research`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Restate the narrow role this skill is allowed to perform for Shell Inspection.
