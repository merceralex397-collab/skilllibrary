---
name: docs-handoff-codex-c
description: Synchronizes documentation and handoff surfaces after a ticket or milestone is complete. Use this when the user or repo work clearly points at docs handoff or a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for tasks outside the narrow docs handoff role.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at docs handoff
- a task in the "Agent Role Candidate Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around docs handoff

# Do not use this skill when

- the task is really about tasks outside the narrow docs handoff role
- If the task is more specifically about `security-review` or `qa-validation`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Restate the narrow role this skill is allowed to perform for Docs Handoff.
