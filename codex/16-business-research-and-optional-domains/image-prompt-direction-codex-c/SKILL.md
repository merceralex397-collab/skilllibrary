---
name: image-prompt-direction-codex-c
description: Creates tighter visual prompting and art-direction guidance for image-generation workflows. Use this when the user or repo work clearly points at image prompt direction or a task in the "Business, Research, and Optional Domain Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for quick factual lookups with no synthesis, comparison, or decision support.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [image, prompt, direction]
---

# Purpose

Creates tighter visual prompting and art-direction guidance for image-generation workflows.

# When to use this skill

Use this skill when:

- the user or repo work clearly points at image prompt direction
...
- a plan, ticket, or repo state would benefit from explicit guardrails around image prompt direction

# Do not use this skill when

- the task is really about quick factual lookups with no synthesis, comparison, or decision support
- If the task is more specifically about `business-idea-evaluation` or `starcraft-data-analysis`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Define the exact question and decision boundary for Image Prompt Direction.
