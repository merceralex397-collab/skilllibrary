---
name: overlay-generator-codex-c
description: Produces host-specific overlays for OpenAI, Gemini, OpenCode, Copilot, or other clients from a shared skill source. Use this when the user or repo work clearly points at overlay generator or a task in the "Package Scaffolding Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for one-off edits that do not benefit from a reusable workflow or package contract.
license: Apache-2.0
compatibility:
  clients: [openai-codex, gemini-cli, opencode, github-copilot]
metadata:
  owner: codex
  domain: overlay-generator
  maturity: draft
  risk: low
  tags: [overlay, generator]
---
...
- the user or repo work clearly points at overlay generator
- a task in the "Package Scaffolding Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around overlay generator

# Do not use this skill when

- the task is really about one-off edits that do not benefit from a reusable workflow or package contract
- If the task is more specifically about `skill-eval-runner` or `skill-description-optimizer`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Normalize the inputs, constraints, and success conditions before generating anything for Overlay Generator.
