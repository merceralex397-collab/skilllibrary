---
name: image-heavy-pdfs-codex-c
description: Specializes PDF handling when the content is primarily diagrams, screenshots, or images rather than text. Use this when the user or repo work clearly points at image-heavy pdfs or a task in the "Docs, Artifacts, and Media Skills" family needs repeatable procedure rather than ad hoc prompting. Do not use for implementation tasks where no artifact or communication deliverable is needed.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at image-heavy pdfs
- a task in the "Docs, Artifacts, and Media Skills" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around image-heavy pdfs

# Do not use this skill when

- the task is really about implementation tasks where no artifact or communication deliverable is needed
- If the task is more specifically about `image-editor` or `document-to-structured-data`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Clarify the audience, artifact type, and decision the document for Image-Heavy PDFs must support.
