---
name: collaboration-checkpoints-codex-c
description: Introduces synchronization moments where agents reconcile assumptions, artifacts, and blockers. Use this when the user or repo work clearly points at collaboration checkpoints or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [collaboration, checkpoints]
---
...
- the user or repo work clearly points at collaboration checkpoints
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around collaboration checkpoints

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `manager-hierarchy-design` or `swarm-patterns`, prefer that skill instead.
