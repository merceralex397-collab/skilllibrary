---
name: panel-of-experts-codex-c
description: Runs a structured multi-perspective pass over a problem before choosing a course of action. Use this when the user or repo work clearly points at panel of experts or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at panel of experts
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around panel of experts

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `parallel-lane-safety` or `artifact-contracts`, prefer that skill instead.
