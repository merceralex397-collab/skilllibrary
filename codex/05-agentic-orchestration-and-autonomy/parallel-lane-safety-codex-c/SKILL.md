---
name: parallel-lane-safety-codex-c
description: Determines which tickets or change sets can safely proceed in parallel and which ones must serialize. Use this when the user or repo work clearly points at parallel lane safety or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at parallel lane safety
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around parallel lane safety

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `workflow-state-memory` or `autonomous-run-control`, prefer that skill instead.
