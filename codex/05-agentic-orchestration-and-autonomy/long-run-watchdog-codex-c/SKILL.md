---
name: long-run-watchdog-codex-c
description: Monitors autonomous workflows for loops, inactivity, repeated errors, or missing outputs and intervenes with a bounded recovery policy. Use this when the user or repo work clearly points at long-run watchdog or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at long-run watchdog
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around long-run watchdog

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `panel-of-experts` or `verification-before-advance`, prefer that skill instead.
