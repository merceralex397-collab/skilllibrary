---
name: subagent-research-patterns-codex-c
description: Structures how narrow research lanes gather evidence, summarize it, and hand it back without mutating repo state. Use this when the user or repo work clearly points at subagent research patterns or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [subagent, research, patterns]
---
...
- the user or repo work clearly points at subagent research patterns
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around subagent research patterns

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `approval-gates` or `session-resume-rehydration`, prefer that skill instead.
