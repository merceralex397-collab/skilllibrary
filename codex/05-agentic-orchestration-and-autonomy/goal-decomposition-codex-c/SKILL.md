---
name: goal-decomposition-codex-c
description: Breaks broad goals into staged work units, subgoals, dependencies, and decision checkpoints. Use this when the user or repo work clearly points at goal decomposition or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at goal decomposition
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around goal decomposition

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `long-run-watchdog` or `human-interrupt-handling`, prefer that skill instead.
