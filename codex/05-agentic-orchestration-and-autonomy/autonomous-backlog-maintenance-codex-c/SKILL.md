---
name: autonomous-backlog-maintenance-codex-c
description: Keeps the backlog clean by reclassifying stale, blocked, duplicated, or superseded tickets under defined rules. Use this when the user or repo work clearly points at autonomous backlog maintenance or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [autonomous, backlog, maintenance]
---
...
- the user or repo work clearly points at autonomous backlog maintenance
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around autonomous backlog maintenance

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `collaboration-checkpoints` or `multi-agent-debugging`, prefer that skill instead.
