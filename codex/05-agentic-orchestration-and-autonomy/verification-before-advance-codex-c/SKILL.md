---
name: verification-before-advance-codex-c
description: Requires proof that a stage is actually complete before moving to the next one. Use this when the user or repo work clearly points at verification before advance or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
metadata:
  ...
  maturity: draft
  risk: low
  tags: [verification, before, advance]
---
...
- the user or repo work clearly points at verification before advance
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around verification before advance

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `artifact-contracts` or `panel-of-experts`, prefer that skill instead.
