---
name: autonomous-run-control-codex-c
description: Defines how autonomous runs start, checkpoint, pause, recover, and terminate safely. Use this when the user or repo work clearly points at autonomous run control or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at autonomous run control
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around autonomous run control

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `subagent-research-patterns` or `workflow-state-memory`, prefer that skill instead.
