---
name: multi-agent-debugging-codex-c
description: Diagnoses whether a failure came from the prompt layer, routing, state, tooling, or the agents themselves. Use this when the user or repo work clearly points at multi-agent debugging or a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting. Do not use for small single-agent tasks where delegation overhead would be larger than the work.
license: Apache-2.0
compatibility:
...
- the user or repo work clearly points at multi-agent debugging
- a task in the "Agentic Orchestration and Autonomy" family needs repeatable procedure rather than ad hoc prompting
- a plan, ticket, or repo state would benefit from explicit guardrails around multi-agent debugging

# Do not use this skill when

- the task is really about small single-agent tasks where delegation overhead would be larger than the work
- If the task is more specifically about `swarm-patterns` or `collaboration-checkpoints`, prefer that skill instead.
