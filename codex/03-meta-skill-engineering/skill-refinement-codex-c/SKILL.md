...
- a plan, ticket, or repo state would benefit from explicit guardrails around skill refinement

# Do not use this skill when

- the task is really about one-off edits that do not benefit from a reusable workflow or package contract
- If the task is more specifically about `skill-creator` or `skill-adaptation`, prefer that skill instead.
- the relevant files, runtime, or deliverable type are already covered by a more specific active skill

# Operating procedure

1. Normalize the inputs, constraints, and success conditions before generating anything for Skill Refinement.
