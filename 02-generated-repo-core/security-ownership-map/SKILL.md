...
# Purpose
Build a bipartite graph of people and files from git history, compute ownership risk (bus factor, orphaned sensitive code), and export graph artifacts for Neo4j/Gephi. Also build a file co-change graph (Jaccard similarity on shared commits) to cluster files by how they move together while ignoring large, noisy commits.
...
2. **Configure sensitivity rules** — use defaults or provide a CSV config.
...
4. **Review communities** — computed by default; graphml output is optional (`--graphml`).
...
# References
- `references/neo4j-import.md` — Neo4j import Cypher and visualization tips

# Failure handling
- **Git log too large**: Narrow with `--since` or `--until` to limit commit range. For very large repos (>100k commits), use `--since "12 months ago"` as a starting point.
- **Missing networkx**: Install with `pip install networkx`. Community detection requires it; the script will fail without it.
- **Empty repo or no commits**: Report "No git history to analyze" and exit gracefully.
- **CODEOWNERS drift**: Compare `summary.json` against CODEOWNERS to highlight ownership drift; flag files with no CODEOWNERS match.
- **Too many supernode commits**: Increase `--cochange-max-files` threshold or use `--author-exclude-regex` to filter bot commits.
